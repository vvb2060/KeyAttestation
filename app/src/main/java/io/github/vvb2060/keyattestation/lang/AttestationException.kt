...
class AttestationException(private val code: Int, cause: Throwable) : RuntimeException(cause) {
        ...
        get() {
            return when (code) {
                ...
                CODE_DEVICEIDS_UNAVAILABLE -> {
                    R.string.error_deviceids_unavailable
                }
                CODE_OUT_OF_KEYS -> {
                    R.string.error_out_of_keys
                }
                CODE_OUT_OF_KEYS_TRANSIENT -> {
                    R.string.error_out_of_keys_transient
                }
                CODE_UNAVAILABLE_TRANSIENT -> {
                    R.string.error_unavailable_transient
                }
                else -> {
    ...
    val descriptionResId: Int
        get() {
            return when (code) {
                CODE_UNAVAILABLE -> {
                    R.string.error_unavailable_summary
                }
                CODE_CERT_NOT_TRUSTED -> {
                ...
                CODE_DEVICEIDS_UNAVAILABLE -> {
                    R.string.error_deviceids_unavailable_summary
                }
                CODE_OUT_OF_KEYS -> {
                    R.string.error_out_of_keys_summary
                }
                CODE_OUT_OF_KEYS_TRANSIENT -> {
                    R.string.error_out_of_keys_transient_summary
                }
                CODE_UNAVAILABLE_TRANSIENT -> {
                    R.string.error_unavailable_transient_summary
                }
                else -> {
                    R.string.error_unknown
                }
